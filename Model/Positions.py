...
def findFirstBloodV5(split: str, week: str, events: dict, json_data: dict) -> pd.DataFrame:
    ...
    for event in events:
        ...
        if event.get('killType') == 'KILL_FIRST_BLOOD':
            ...
            if participantId <= 5:
                return pd.DataFrame({'Split': split, 'Week': week, 'Team': killer.split(' ')[0], 'Player': killer.split(' ')[0], 'X': position['x'], 'Y': position['y']}, index=[0])

            else:
                return pd.DataFrame({'Split': split, 'Week': week, 'Team': killer.split(' ')[0], 'Player': killer.split(' ')[0], 'X': position['y'], 'Y': position['x']}, index=[0])
